...
inline float3 ray_color_no_bvh(
    ...
    for (uint depth = 0; depth < max_depth; depth++) {
        ...
        // 测试所有球体（跳过仅作为体积雾边界的球体）
        for (uint i = 0; i < sphere_count; i++) {
            // 跳过材质索引为 UINT32_MAX 的边界球体
            if (spheres[i].material_index == 0xFFFFFFFFu) {
                continue;
            }
            HitRecord temp_rec;
...
inline float3 ray_color_bvh(
    ...
    for (uint depth = 0; depth < max_depth; depth++) {
        ...
        // 体积雾（暂时不使用 BVH，直接遍历）
        if (constant_medium_count > 0) {
            HitRecord temp_rec;
            float closest_so_far = hit_anything ? rec.t : 1e10f;
            for (uint i = 0; i < constant_medium_count; i++) {
                if (constant_medium_hit(constant_mediums[i], spheres, quads, transforms,
                                      current_ray, 0.001f, closest_so_far, &temp_rec, rng)) {
                    hit_anything = true;
...
kernel void simple_raytrace(
    ...
    device const GPUBVHNode* bvh_nodes [[buffer(8)]],
    device const uint* geometry_indices [[buffer(9)]],
