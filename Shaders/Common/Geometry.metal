...
/// 2. 计算光线在边界内的距离
/// 3. 使用负指数分布随机决定散射点位置
/// 4. 如果散射点在边界内，返回命中，使用 isotropic 材质
inline bool constant_medium_hit(
    GPUConstantMedium medium,
    ...
    // 步骤1: 检查光线是否穿过边界几何体（第一次相交）
    bool hit_boundary = false;

    if (medium.boundary_type == 0) {
        // Sphere boundary
        GPUSphere boundary = spheres[medium.boundary_index];
        hit_boundary = sphere_hit(boundary, transforms, r, -1e38f, 1e38f, &rec1);
    }
    // 未来可扩展其他边界类型
    ...
    // 步骤2: 检查光线从第一个交点继续穿过边界（第二次相交）
    if (medium.boundary_type == 0) {
        GPUSphere boundary = spheres[medium.boundary_index];
        if (!sphere_hit(boundary, transforms, r, rec1.t + 0.0001f, 1e38f, &rec2)) {
            return false;
        }
    }
    ...
    rec->normal = float3(1.0f, 0.0f, 0.0f);  // 任意法线
    rec->front_face = true;                  // 任意正面
    rec->material_index = medium.material_index;
    rec->u = 0.0f;
    rec->v = 0.0f;
