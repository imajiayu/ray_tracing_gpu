...
print("[Scene] ✓ \(gpuSpheres.count) spheres, \(gpuQuads.count) quads, \(gpuMaterials.count) materials, \(gpuTextures.count) textures, \(gpuTransforms.count) transforms, \(scene.imageTextures.count) image textures")
...
if cmdArgs.mode == "window" {
    // 窗口模式 - 未实现
    print("❌ Window mode is not yet implemented")
    exit(1)
} else {
    // Image 模式 - 离线渲染
