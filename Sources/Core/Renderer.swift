...
class Renderer {
    ...
    private func createBuffers(
        ...
        // 创建体积雾缓冲区（如果有）
        let constantMediumBuffer: MTLBuffer
        if constantMediums.isEmpty {
            guard let buffer = context.device.makeBuffer(length: MemoryLayout<GPUConstantMedium>.stride, options: []) else {
                return nil
            }
            constantMediumBuffer = buffer
        } else {
            guard let buffer = context.makeBuffer(array: constantMediums) else {
                return nil
            }
            constantMediumBuffer = buffer
        }
        ...
        return GPUBuffers(
            ...
            constantMediumBuffer: constantMediumBuffer,
