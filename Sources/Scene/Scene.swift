...
struct Scene {
    ...
    /// 转换为 GPU 数据
    func toGPU() -> (spheres: [GPUSphere], quads: [GPUQuad], materials: [GPUMaterial], textures: [GPUTexture], transforms: [GPUTransform]) {
        let (gpuSpheres, gpuQuads) = geometry.toGPU()
        let gpuMaterials = materials.map { $0.toGPU() }
        return (gpuSpheres, gpuQuads, gpuMaterials, textures, transforms)
    }
